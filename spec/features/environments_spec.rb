...
RSpec.describe 'Cannon environment' do
  ...
  describe 'environment helper methods' do
    before(:each) do
      ...
      app.configure(:potatoes) {}
      Cannon.configure(:celeries) {}
      app.configure(:onions, :lettuce, :pickles)
    end

    it 'creates a helper method for current environment' do
      expect(app.env.carrots?).to be(true)
    end

    it 'creates helper methods for single configured environments' do
      expect(app.env.potatoes?).to be(false)
      expect(app.env.celeries?).to be(false)
    end

    it 'creates helper methods for multi configured environments' do
      expect(app.env.onions?).to be(false)
      expect(app.env.lettuce?).to be(false)
      expect(app.env.pickles?).to be(false)
    end
  end
